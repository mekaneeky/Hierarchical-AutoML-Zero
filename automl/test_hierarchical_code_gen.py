...
class HierarchicalGenomeUnpacker:
    ...
    def unpack_hierarchical_genome(self, hierarchical_genome, gene_idx, selective=False):
        ...
        if selective:
            ...
            for sub_gene_idx in gene_to_expand.gene:
                try:
                    code_lines.extend(self._unpack_selective(hierarchical_genome, top_level-1, sub_gene_idx)) #TODO add index/gene
                except:
                    breakpoint()
        else:
            for level in range(hierarchical_genome.num_meta_levels):
                code_lines.extend(self._unpack_level(level, hierarchical_genome.genomes[level]))
        try:
    ...
    def _create_main_function(self, hierarchical_genome, gene_idx, selective):
        ...
        if top_level == 0:
            ...
            code_lines.append(f"    result = execute_level_{top_level}_genome_0(memory)")
        else:
            ...
            for i, lower_genome_index in enumerate(top_genome.gene):
                if 0 <= lower_genome_index < len(hierarchical_genome.genomes[top_level-1]):
                    ...
                    print(f"Warning: Invalid lower genome index {lower_genome_index} at top level. Skipping.")
...
hierarchical_genome = HierarchicalGenome(num_meta_levels, genome_length, hierarchical_memory,decoder , population_size)

unpacker = HierarchicalGenomeUnpacker()
generated_code = unpacker.unpack_hierarchical_genome(hierarchical_genome, 0, selective=True)#TODO add metalevel + index
...
# Create a simple hierarchical memory structure
#hierarchical_memory = [[0] * 10 for _ in range(2)]

# Run the hierarchical genome with some input data
input_data = torch.tensor(5.0)
result = execute_hierarchical_genome(input_data, hierarchical_memory)
