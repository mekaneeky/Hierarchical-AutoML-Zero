...
if __name__ == "__main__":
    ...
    # Fetch a sample from the MNIST dataset for input_data and desired_output
    input_data_sample, target_sample = next(iter(train_loader))
    input_data = input_data_sample.squeeze().numpy()  # Convert to numpy array and remove batch dimension
    desired_outputs = target_sample  # For demonstration purposes, use the same image as desired output

    #input_data = torch.tensor([[3, 3, 3],[3, 3, 3],[3, 3, 3]])
    #desired_outputs = (input_data//1)*2 + input_data
